...
class LensAIPDF(FPDF):
    ...
    def footer(self):
        ...
        self.cell(
            0, 10,
            _safe_latin1("Generated by LensAI | Simpleem Meeting Intelligence"),
            align="C",
        )
...
async def _generate_engagement_chart(video_id: str) -> Optional[str]:
...
async def generate_pdf_report(video_id: str) -> bytes:
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Meeting notes (table may not exist in online DB)
        try:
            cursor = await db.execute(
...
def _ensure_space(pdf: LensAIPDF, needed_height: float):
...
def _section_heading(pdf: LensAIPDF, title: str):
...
def _draw_table(pdf: LensAIPDF, headers: list, col_widths: list, rows: list,
                multi_cell_cols: Optional[set] = None):
...
def _estimate_multicell_height(pdf: LensAIPDF, text: str, col_width: float,
                                line_h: float) -> float:
...
def _draw_table_multicell(pdf: LensAIPDF, headers: list, col_widths: list,
                           rows: list, wrap_cols: Optional[set] = None):
...
def _build_pdf(
    ...
    pdf.set_text_color(*HEADING_BLUE)
    pdf.cell(0, 16, _safe_latin1("LensAI Meeting Analysis Report"),
    ...
    for p in participants:
        p_rows.append([
            ...
            f'{round(p["sentiment_score"])}%' if p["sentiment_score"] is not None else "N/A",
            f'{round(p["clarity_score"])}%',
            f'{round(p["rapport_score"])}%',
            f'{round(p["energy_score"])}%',
            f'{round(p["speaking_pct"])}%',
        ])
    _draw_table_multicell(pdf, p_headers, p_widths, p_rows, wrap_cols={0})
    pdf.ln(4)
